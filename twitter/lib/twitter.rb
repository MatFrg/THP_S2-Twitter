...
def login_twitter
	client = Twitter::REST::Client.new do |config| #REST ou Streaming
...
def say_bonjour(client)
	...
	journalist.sample(4).each {|j| client.update("#bonjour_monde#{j}")}
end

def liker_bonjour(client)
...
def like_follow_bonjour(client)
